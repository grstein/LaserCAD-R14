(function (LaserCAD) {
  ...
  function preview(p, applyOrtho) {
    ...
    for (let i = 0; i < points.length - 1; i++) {
      ...
      n.setAttribute('stroke-dasharray', '4 2'); n.setAttribute('stroke', glow);
    }
  ...
  function commitAll() {
    for (let i = 0; i < points.length - 1; i++) {
      TM().commit(commands().addEntity({ type: 'line', p1: points[i], p2: points[i+1] }));
    }
    points = [];
  }

  LaserCAD.tools.polylineTool = {
    id: 'polyline',
    prompt: 'POLYLINE  Specify next point or Esc to finish:',
    onArm(state) { points = []; state.setToolState('armed'); },
    onCancel()   {
