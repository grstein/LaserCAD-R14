(function (LaserCAD) {
  ...
  LaserCAD.tools.extendTool = {
    ...
    onPointerDown(e, state) {
      ...
      let newGeom = null;
      if (hit.type === 'line' && boundary.type === 'line')   newGeom = extendLineToLine(hit, boundary, p);
      else if (hit.type === 'line' && boundary.type === 'circle') newGeom = extendLineToCircle(hit, boundary, p);
      if (!newGeom) {
        LaserCAD.bus.emit('command:error', { raw: 'extend', message: '! No intersection found' });
        return;
      }
