(function (LaserCAD) {
  ...
  function commitRect(p2) {
    ...
    lines.forEach(function (l) { TM().commit(commands().addEntity(l)); });
    firstPoint = null;
    LaserCAD.tools.toolManager.cancel();
  }

  LaserCAD.tools.rectTool = {
    id: 'rect',
    prompt: 'RECT  Specify first corner:',
    onArm(state)   { firstPoint = null; state.setToolState('armed'); },
    onCancel()     { firstPoint = null; const sr = TM().getSvgRoot(); if (sr) LaserCAD.render.entityRenderers.clearPreview(sr); },
    onPointerMove(e) { if (firstPoint) previewRect({ x: e.worldX, y: e.worldY }); },
    onPointerDown(e, state) {
      const p = { x: e.worldX, y: e.worldY };
      if (!firstPoint) { firstPoint = p; state.setToolState('preview'); }
      else commitRect(p);
    },
