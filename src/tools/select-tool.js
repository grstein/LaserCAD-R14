(function (LaserCAD) {
  ...
  function hitTest(p, entity, tol) {
    ...
    if (entity.type === 'circle') {
      const dx = p.x - entity.center.x, dy = p.y - entity.center.y;
    ...
    if (entity.type === 'arc') {
      ...
      return LaserCAD.core.geometry.arc.containsAngle(entity, ang);
    }
    return false;
  }
  function entityInBox(entity, minX, minY, maxX, maxY) {
    let bb;
    if (entity.type === 'line')   bb = LaserCAD.core.geometry.line.bbox(entity);
    else if (entity.type === 'circle') bb = LaserCAD.core.geometry.circle.bbox(entity);
    else if (entity.type === 'arc') bb = LaserCAD.core.geometry.arc.bbox(entity);
    if (!bb) return false;
  ...
  function renderBox() {
    ...
    r.setAttribute('stroke', c); r.setAttribute('stroke-width', '0.1');
    r.setAttribute('fill', c); r.setAttribute('fill-opacity', '0.08');
  ...
  LaserCAD.tools.selectTool = {
    ...
    onArm(state)  { boxStart = null; boxRect = null; state.setToolState('armed'); },
    onCancel()    { boxStart = null; boxRect = null; const sr = TM().getSvgRoot(); if (sr) LaserCAD.render.entityRenderers.clearPreview(sr); },
    onPointerDown(e, state) {
      ...
      for (let i = state.entities.length - 1; i >= 0; i--) {
        ...
        if (hitTest(p, ent, tol)) {
          ...
          TM().commit(commands().setSelection(sel));
          return;
        }
      }
    ...
    onPointerUp(e, state) {
      ...
      const minX = Math.min(boxRect.x1, boxRect.x2), minY = Math.min(boxRect.y1, boxRect.y2);
      const maxX = Math.max(boxRect.x1, boxRect.x2), maxY = Math.max(boxRect.y1, boxRect.y2);
      // Drag pequeno (clique sem arrasto): limpa seleção
      const tinyMm = 5 / Math.max(LaserCAD.render.camera.pxPerMm(state.camera), 0.0001);
      if (Math.abs(maxX - minX) < tinyMm && Math.abs(maxY - minY) < tinyMm) {
        TM().commit(commands().setSelection([]));
      } else {
        const picked = state.entities.filter(function (ent) { return entityInBox(ent, minX, minY, maxX, maxY); }).map(function (ent) { return ent.id; });
