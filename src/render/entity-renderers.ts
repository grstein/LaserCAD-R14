...
function laserColor() {
  return (
    getComputedStyle(document.documentElement).getPropertyValue('--laser-450').trim() || '#6E00FF'
  );
}
...
export const entityRenderers = {
  ...
  renderArc(entity, parent) {
    const ends = arc.endpoints(entity);
    const da = entity.ccw
    ...
    const d =
      ...
      ' ' +
      ends.end.y;
    const node = el('path', { d: d, stroke: laserColor(), 'stroke-width': '1.5', fill: 'none' });
    node.setAttribute('vector-effect', 'non-scaling-stroke');
    parent.appendChild(node);
    return node;
  },
