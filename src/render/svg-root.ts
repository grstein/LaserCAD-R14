...
export const svgRoot = {
  ...
  mount(hostOrSelector) {
    const host =
      typeof hostOrSelector === 'string' ? document.querySelector(hostOrSelector) : hostOrSelector;
    ...
    const handle = {
      ...
      refreshViewBox() {
        ...
        const pxPerMm = camera.pxPerMm(cam);
      ...
      getLayer(name) {
        return svg.querySelector('#' + name);
      },
    };

    bus.on('camera:changed', handle.refreshViewBox);
    bus.on('viewport:resized', handle.refreshViewBox);
