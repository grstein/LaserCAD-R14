(function (LaserCAD) {
  ...
  ns.grid = {
    mount(svgRoot) {
      ...
      function render() {
        if (!LaserCAD.app.state.toggles.grid) {
          clear(layerGrid);
          renderAxes();
          return;
        }
      ...
      function renderAxes() {
        ...
        const pxPerMm = LaserCAD.render.camera.pxPerMm(LaserCAD.app.state.camera);
        const w = 1 / pxPerMm;
        const xAxis = el('line', { x1: vb.minX, y1: 0, x2: vb.minX + vb.w, y2: 0, stroke: colorAxis, 'stroke-width': w });
        const yAxis = el('line', { x1: 0, y1: vb.minY, x2: 0, y2: vb.minY + vb.h, stroke: colorAxis, 'stroke-width': w });
      ...
      function renderLines(parent, vb, step, color, strokeMm) {
        ...
        for (let x = xStart; x <= xEnd; x += step) {
          const line = el('line', { x1: x, y1: vb.minY, x2: x, y2: vb.minY + vb.h, stroke: color, 'stroke-width': strokeMm });
          line.setAttribute('vector-effect', 'non-scaling-stroke');
          parent.appendChild(line);
        }
        for (let y = yStart; y <= yEnd; y += step) {
          const line = el('line', { x1: vb.minX, y1: y, x2: vb.minX + vb.w, y2: y, stroke: color, 'stroke-width': strokeMm });
          line.setAttribute('vector-effect', 'non-scaling-stroke');
          parent.appendChild(line);
        }
      }
      ...
      LaserCAD.bus.on('camera:changed',   render);
      LaserCAD.bus.on('viewport:resized', render);
      LaserCAD.bus.on('toggle:changed',   function (p) { if (p && p.name === 'grid') render(); });
      LaserCAD.bus.on('app:ready',        render);
