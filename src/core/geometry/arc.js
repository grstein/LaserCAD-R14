(function (LaserCAD) {
  ...
  ns.arc = {
    ...
    /** @param {Arc} arc @returns {{minX:number,minY:number,maxX:number,maxY:number}} */
    bbox(arc) {
      const cx = arc.center.x, cy = arc.center.y, r = arc.r;
      const sx = cx + r * Math.cos(arc.startAngle), sy = cy + r * Math.sin(arc.startAngle);
      const ex = cx + r * Math.cos(arc.endAngle),   ey = cy + r * Math.sin(arc.endAngle);
      ...
      for (let i = 0; i < cardinals.length; i++) {
        if (ns.arc.containsAngle(arc, cardinals[i].angle)) {
