(function (LaserCAD) {
  ...
  function entityToXml(e, stroke, strokeWidth) {
    if (e.type === 'line') {
      return '  <line x1="' + fmt(e.p1.x) + '" y1="' + fmt(e.p1.y) + '" x2="' + fmt(e.p2.x) + '" y2="' + fmt(e.p2.y) + '"/>';
    }
    if (e.type === 'circle') {
      return '  <circle cx="' + fmt(e.center.x) + '" cy="' + fmt(e.center.y) + '" r="' + fmt(e.r) + '"/>';
    }
    if (e.type === 'arc') {
      const ends = LaserCAD.core.geometry.arc.endpoints(e);
      const da = e.ccw
        ? ((e.endAngle - e.startAngle + 2*Math.PI) % (2*Math.PI))
        : ((e.startAngle - e.endAngle + 2*Math.PI) % (2*Math.PI));
      const large = da > Math.PI ? 1 : 0;
      const sweep = e.ccw ? 1 : 0;
      return '  <path d="M ' + fmt(ends.start.x) + ' ' + fmt(ends.start.y) +
             ' A ' + fmt(e.r) + ' ' + fmt(e.r) + ' 0 ' + large + ' ' + sweep + ' ' +
             fmt(ends.end.x) + ' ' + fmt(ends.end.y) + '"/>';
    }
    return '';
  }
  ...
  function serialize(state, opts) {
    ...
    const strokeWidth = opts.strokeWidth != null ? opts.strokeWidth : 0.1;
    const bounds = state.documentBounds;
    const w = bounds.w, h = bounds.h;

    const xml = [];
    xml.push('<?xml version="1.0" encoding="UTF-8"?>');
    xml.push('<svg xmlns="http://www.w3.org/2000/svg" width="' + fmt(w) + 'mm" height="' + fmt(h) + 'mm" viewBox="0 0 ' + fmt(w) + ' ' + fmt(h) + '">');
    xml.push('  <g id="' + preset.toUpperCase() + '" fill="none" stroke="' + stroke + '" stroke-width="' + fmt(strokeWidth) + '">');
  ...
  LaserCAD.io.exportSvg = { serialize: serialize };
